<?php
/**
 * Table Definition for view_asiento_contable
 */
require_once 'DB/DataObject.php';

class DataObjects_View_asiento_contable extends DB_DataObject 
{
    ###START_AUTOCODE
    /* the code below is auto generated do not remove the above tag */

    public $__table = 'view_asiento_contable';    // table name
    public $asiento_descripcion_nombre;      // varchar(255)  not_null
    public $asiento_empresa_numero_cuenta;    // int(255)  not_null group_by
    public $empresa_nombre;                  // varchar(200)  not_null
    public $maestro_concepto_nombre;         // varchar(250)  not_null
    public $maestro_concepto_codigo;         // varchar(45)  not_null
    public $maestro_concepto_id;             // int(11)  not_null primary_key auto_increment group_by
    public $empresa_id;                      // int(11)  not_null primary_key auto_increment group_by

    /* Static get */
    function staticGet($k,$v=NULL) { return DB_DataObject::staticGet('DataObjects_View_asiento_contable',$k,$v); }

    /* the code above is auto generated do not remove the tag below */
    ###END_AUTOCODE
}
