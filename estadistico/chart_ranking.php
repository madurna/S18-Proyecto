<?php
   ...
   //creo formulario
    ...
    //arreglo orden
    $v_orden = array('1'=>'Minimo', '2'=>'Maximo');
    ...
    //nombre    
    $frm ->addElement('select','orden','Orden: ',$v_orden,array('id' => 'orden'));
    ...
    $body = '<h2>Ranking bruto/neto por empresa ( '.$anio.' )</h2><br /><div align="center"><br/>'.$frm->toHTML().'</div>';

    if ($aceptar == 'Generar')
    {
        $do_view_estadistica_ranking_neto_bruto= DB_DataObject::factory('view_estadistica_ranking_neto_bruto');

        if ($tipo_salida)
            $do_view_estadistica_ranking_neto_bruto -> whereAdd(" detalle_liquidacion_concepto_id = $tipo_salida");

        if ($anio)
            $do_view_estadistica_ranking_neto_bruto -> whereAdd(" liquidacion_anio = $anio");
        
        $do_view_estadistica_ranking_neto_bruto -> find();
        ...
        while ( $do_view_estadistica_ranking_neto_bruto -> fetch()){
            $empresa = $do_view_estadistica_ranking_neto_bruto -> liquidacion_empresa_id;
            $mes = $do_view_estadistica_ranking_neto_bruto -> liquidacion_mes;
            if ( $orden == 1)
              $data[$empresa][$mes] = $do_view_estadistica_ranking_neto_bruto -> detalle_liquidacion_minimo;  
            elseif(  $orden == 2 ) 
              $data[$empresa][$mes] = $do_view_estadistica_ranking_neto_bruto -> detalle_liquidacion_maximo;
        }
