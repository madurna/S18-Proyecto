...
//        }
	...
	function validaSemestres($fields = 0) {
		...
		function encuentraCategoria($fields) {
		if ($fields['categoria_convenio_empresa_id'][1] == 0){
			{return array('categoria_convenio_empresa_id' => 'Seleccione empresa/convenio');}
		}
		else{
			$do = DB_DataObject::factory('categoria_convenio_empresa');
			$do -> categoria_nombre = $fields['categoria_nombre'];
			$do -> categoria_convenio_empresa_id = $fields['categoria_convenio_empresa_i'][1];
			$do -> categoria_codigo = $fields['categoria_codigo'];
			if ($do->find(true)) 
			  {return array('categoria_codigo' => 'Ya existe la categor&iacute;a para la empresa/convenio seleccionada');}
			else 
			  {return true;}
		}
	}

	function encuentraEmpleado($fields) {
		if ($fields['empleado_empresa_i'][2] == 0){
			{return array('empleado_empresa_i' => 'Seleccione empresa/convenio/categor&iacute;a');}
		}
		else{
			$do = DB_DataObject::factory('empleado');
			$do -> empleado_legajo = $fields['empleado_legajo'];
			$do -> empleado_empresa_id = $fields['empleado_empresa_i'][0];
			if ($do->find(true)) 
			  {return array('empleado_legajo' => 'Ya existe el legajo ingresado para la empresa seleccionada');}
			else 
			  {return true;}
		}
	}
