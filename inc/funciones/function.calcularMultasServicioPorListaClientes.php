...
function calcularMultasServicioPorListaClientes($arch_id = 0, $lista_clientes_id = array(),$fuerza_mayor = 0,$mdb2 = null)
{
...
//    }
    ...
    if(PROCESAR){
        ...
        //$clientes = implode(",",$lista_clientes_id);
        echo_debug('Se procesan cortes para los clientes: '.$lista_clientes_id.' en cortes por usuario <br>');
...
//                AND cli_cod_suministro = ".$cod_sum;
        ...
        $sql = "SELECT cortesusua_cli_id FROM cortes_por_usuario
                WHERE cortesusua_arch_id = ".$arch_id."
                AND cortesusua_cli_id IN (".$lista_clientes_id.")";
        ...
        $lotes_clientes = array_chunk(array_unique($lista_clientes),CANTCLIENTES);
        $cant_lotes_clientes = count($lotes_clientes);
        
        //echo_debug('Proceso en lotes de : '.CANTCLIENTES.' ');

        for($i= 0; $i < $cant_lotes_clientes; $i++){
        //for($i= 0; $i < $lista_clientes; $i++){
            $multas_por_usuario_calculada = array();
            $j = 0;

            //Calculo de multas
            $penaliza = calcularEnergiaNoSuministrada($minimoMinutos,$minimoHoras,$sem_id,$emp_id,$tipo_distribuidora_id,$zona_id,$cliente_id,$arch_id,$lotes_clientes[$i],$fuerza_mayor,$mdb2);
            //$penaliza = calcularEnergiaNoSuministrada($minimoMinutos,$minimoHoras,$sem_id,$emp_id,$tipo_distribuidora_id,$zona_id,$cliente_id,$arch_id,$lista_clientes[$i],$fuerza_mayor,$mdb2);
