<?php
	...
	$frm ->addElement('text','apellido','Apellido:', array('id' => 'apellido', 'value'=>$do_obrero -> obrero_apellido));
	$frm ->addElement('text','nombre','Nombre:', array('id' => 'nombre', 'value'=>$do_obrero -> obrero_nombre));
	$frm ->addElement('text','dni','Documento:', array('id' => 'dni', 'value'=> $do_obrero -> obrero_nro_doc));
	$frm ->freeze(nombre);
	$frm ->freeze(apellido);
	$frm ->freeze(dni);
	...
	$frm ->addElement('hidden','hidden_obrero', $obrero_id, array('id' => 'hidden_obrero'));
	...
	$obrero_post_id = $_POST['hidden_obrero'];
		...
		for ($i = 1; $i <= $cantidad_adjuntos; $i++) {
			...
			if ($file->isValid()) {
				...
				if ($campo_adjunto == 'adjunto_1'){
					$carpeta = WWW_PATH.'/adjuntos/obrero/'.$obrero_post_id;
					mkdir($carpeta, 0777);
				}
				...
				// Ruta del archivo
				$ruta='../adjuntos/obrero/'.$obrero_post_id.'/'.$nom;
				
				// Muevo el archivo a la carpeta correspondiente
				$file->moveTo($carpeta);
				chmod($ruta, 0755);
				// Inserto en la tabla "adjuntos"
				$do_adjuntos = DB_DataObject::factory('adjuntos_obrero');
				$do_adjuntos -> adjuntos_obrero_tipo_adjunto_id = $_POST['tipo_adjunto_'.$i];
				$do_adjuntos -> adjuntos_obrero_obrero_id = $obrero_post_id;
				$do_adjuntos -> adjuntos_obrero_direccion = $ruta;
				$do_adjuntos -> adjuntos_obrero_descripcion = $_POST['descripcion_'.$i];
				$do_adjuntos -> adjuntos_obrero_nombre = $nom;
	...
	$tpl = new tpl();
	$titulo_grilla = 'Subir adjuntos obrero';
	$body =
	...
	$tpl->assign('secTitulo', WEB_SECCION . ' - Subir adjuntos obrero');
