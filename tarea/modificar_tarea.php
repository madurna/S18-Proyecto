<?php
	...
	//recupero el id de la tarea a modificar
	$tarea_id = $_GET['contenido'];
	
	//recupero el nombre que tenia originalmente en la base
	$do_tarea = DB_DataObject::factory('tarea');
	$do_tarea -> tarea_id = $tarea_id;
	$do_tarea -> fb_fieldsToRender = array('tarea_descripcion','tarea_peso','tarea_baja');
	$do_tarea -> find(true);
	
	if (!$do_tarea->find(true)) {
	...
	if($frm->validate()){
		...
		if ($id){	
			...
			$error = 'Error en la modificaci&oacute;n de la Tarea</b></div>';				
		}
