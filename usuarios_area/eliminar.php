<?php
	...
	$do -> fb_fieldsToRender = array('usarea_usua_id','usarea_area_id');	
		
	if($do->find(true))
		{
		//recupero id del usuario
		$id_usuario = $do -> usarea_usua_id;
		//
	...
	if($frm->validate()){
		...
		if ($delete){
			...
			$do->query('ROLLBACK');
			$error = 'Error en la eliminaci&oacute;n del usuario &aacute;rea</b></div>';				
		}
	...
	$tpl = new tpl();
	$titulo_grilla = 'Eliminar usuario &aacute;rea';
	$body =
	...
	$tpl->assign('secTitulo', WEB_SECCION . ' - Eliminar usuario &aacute;rea');
